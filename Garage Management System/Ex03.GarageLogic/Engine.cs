...
namespace Ex03.GarageLogic
{
    internal abstract class Engine
    {
        ...
        public void AddEnergy(float i_Amount)
        {
            if (i_Amount < 0)
            {
                throw new ArgumentException("Fuel amount cannot be negative", nameof(i_Amount)); //UI
            }
            if (m_CurrentAmount + i_Amount > r_MaxAmount)
            {
                throw new ArgumentException($"Fuel amount exceeds maximum capacity. Current: {m_CurrentAmount}, Adding: {i_Amount}, Max: {r_MaxAmount}"); //UI
            }
